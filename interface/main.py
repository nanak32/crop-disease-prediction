...
GUI Interface — main_gui.py

Runs the Prolog KB via pyswip and presents a full Tkinter GUI.
Disease-by-disease flow: show all 3 symptoms → Yes/No → next disease.

Requirements:
    pip install pyswip
    SWI-Prolog must be installed: https://www.swi-prolog.org/Download.html
"""
...
class ExpertSystemApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Crop Disease Expert System — DCIT 313")
    ...
    def _build_shell(self):
        ...
        tk.Label(hdr, text="🌿 CROP DISEASE EXPERT SYSTEM",
                 font=self.f_title, bg=C["panel"], fg=C["accent"]).pack(side="left", padx=24, pady=14)

        self.lbl_crop_badge = tk.Label(hdr, text="", font=self.f_small,
        ...
        self.progress_bar = tk.Frame(self.progress_frame, bg=C["accent"], height=3, width=0)
    ...
    def _screen_crop_select(self):
        ...
        tk.Label(outer, text="Which crop are you diagnosing?",
                 font=self.f_disease, bg=C["bg"], fg=C["text"]).pack(pady=(0, 8))
        tk.Label(outer, text="Select a crop to begin the diagnostic session.",
                 font=self.f_sub, bg=C["bg"], fg=C["text_dim"]).pack(pady=(0, 36))

        btn_frame = tk.Frame(outer, bg=C["bg"])
        btn_frame.pack()

        for crop, emoji in [("maize", "🌽"), ("tomato", "🍅")]:
            card = tk.Frame(btn_frame, bg=C["card"], bd=0,
                            highlightthickness=2, highlightbackground=C["border"],
                            cursor="hand2")
            card.pack(side="left", padx=16, ipadx=20, ipady=20)

            tk.Label(card, text=emoji, font=tkfont.Font(size=42),
                     bg=C["card"]).pack(pady=(12, 4))
            tk.Label(card, text=crop.upper(), font=self.f_btn,
                     bg=C["card"], fg=C["accent"]).pack(pady=(0, 12))

            # Hover effect
            def _enter(e, c=card):  c.config(highlightbackground=C["accent"])
            def _leave(e, c=card):  c.config(highlightbackground=C["border"])
            def _click(e, cr=crop): self._start_diagnosis(cr)
        ...
        tk.Label(outer,
                 text="DCIT 313  |  Knowledge-Based System  |  Prolog + Python",
                 font=self.f_small, bg=C["bg"], fg=C["border"]).pack(pady=(48, 0))
    ...
    # ── SCREEN: QUESTION (one disease at a time) ─────────────
    ...
    def _screen_question(self):
        ...
        tk.Label(wrapper,
                 ...
                 font=self.f_counter, bg=C["bg"], fg=C["text_dim"]).pack(anchor="w")

        # Progress dots
        dot_row = tk.Frame(wrapper, bg=C["bg"])
        dot_row.pack(anchor="w", pady=(4, 18))
        for i in range(total):
            color = C["accent"] if i < current else (C["border"] if i > self.disease_idx else C["gold"])
            tk.Frame(dot_row, bg=color, width=18 if i == self.disease_idx else 8, height=8).pack(side="left", padx=1)

        # Question card
        card = tk.Frame(wrapper, bg=C["card"],
        ...
        tk.Label(name_bar,
                 ...
                 font=self.f_label, bg=C["accent_dark"], fg=C["text_dim"],
                 anchor="w").pack(fill="x", padx=16, pady=(10, 10))
        ...
        tk.Label(sym_frame,
                 ...
                 font=self.f_label, bg=C["card"], fg=C["text_dim"]).pack(anchor="w", pady=(0, 10))

        for i, symptom in enumerate(info.get("symptoms", []), 1):
            row = tk.Frame(sym_frame, bg=C["symptom_bg"],
                           highlightthickness=1, highlightbackground=C["border"])
            row.pack(fill="x", pady=3)

            tk.Label(row, text=f" {i} ", font=self.f_label,
                     bg=C["accent_dark"], fg=C["white"]).pack(side="left")
            tk.Label(row, text=f"  {symptom}",
                     font=self.f_body, bg=C["symptom_bg"], fg=C["symptom_txt"],
        ...
        yes_btn = self._make_btn(btn_row, "✓  YES — Confirm These Symptoms",
                                 C["accent"], C["accent_dark"],
                                 lambda: self._answer(True, disease_name, info))
        yes_btn.pack(side="left", padx=10, ipadx=12, ipady=8)

        no_btn  = self._make_btn(btn_row, "✗  NO — Not These Symptoms",
                                 C["danger"], C["danger_dark"],
        ...
        tk.Label(wrapper,
                 ...
                 font=self.f_small, bg=C["bg"], fg=C["border"],
                 wraplength=700, justify="center").pack(pady=(6, 0))

    def _make_btn(self, parent, text, bg, active_bg, cmd):
        btn = tk.Button(parent, text=text, font=self.f_btn,
                        bg=bg, fg=C["white"], activebackground=active_bg,
                        activeforeground=C["white"],
                        relief="flat", cursor="hand2",
                        bd=0, command=cmd)
        def _enter(e): btn.config(bg=active_bg)
        def _leave(e): btn.config(bg=bg)
    ...
    def _screen_results(self):
        ...
        canvas = tk.Canvas(self.content, bg=C["bg"], bd=0,
                            highlightthickness=0)
        scrollbar = tk.Scrollbar(self.content, orient="vertical",
        ...
        tk.Label(inner, text=headline, font=self.f_disease,
                 bg=C["bg"], fg=h_color).pack(anchor="w", pady=(0, 4))
        tk.Label(inner,
                 text=f"{self.crop.capitalize()} crop  ·  {total_checked} diseases screened  ·  {num_found} confirmed",
                 font=self.f_sub, bg=C["bg"], fg=C["text_dim"]).pack(anchor="w", pady=(0, 20))

        if num_found == 0:
            no_card = tk.Frame(inner, bg=C["result_neg"],
                               highlightthickness=1, highlightbackground=C["border"])
            no_card.pack(fill="x", pady=8)
            tk.Label(no_card,
                     text=("Your crop did not match any of the known disease patterns.\n"
                            "It may be healthy, or the symptoms may not yet be apparent.\n"
                            "Consider consulting a local agricultural extension officer."),
                     font=self.f_body, bg=C["result_neg"], fg=C["text"],
                     justify="left", wraplength=680).pack(padx=24, pady=20)
        else:
            for i, (disease_name, info) in enumerate(self.confirmed, 1):
                self._render_result_card(inner, i, disease_name, info)

        # Disclaimer
        disc = tk.Frame(inner, bg=C["panel"],
                        highlightthickness=1, highlightbackground=C["border"])
        ...
        tk.Label(disc,
                 ...
                 wraplength=680, justify="left").pack(padx=20, pady=12)

        # Restart button
        self._make_btn(inner, "← Start New Diagnosis",
                       C["accent_dark"], C["accent"],
                       self._restart).pack(pady=16, ipadx=10, ipady=6)

    def _render_result_card(self, parent, idx, disease_name, info):
        card = tk.Frame(parent, bg=C["result_pos"],
                        highlightthickness=1, highlightbackground=C["accent_dark"])
        ...
        tk.Label(title_bar,
                 ...
                 font=self.f_disease, bg=C["accent_dark"], fg=C["gold"],
        ...
        # Confirmed symptoms
        tk.Label(body, text="CONFIRMED SYMPTOMS",
                 font=self.f_label, bg=C["result_pos"], fg=C["text_dim"]).pack(anchor="w")
        for s in info["symptoms"]:
            row = tk.Frame(body, bg=C["symptom_bg"])
            row.pack(fill="x", pady=2)
            tk.Label(row, text="  ✓  ", font=self.f_label,
                     bg=C["accent"], fg=C["white"]).pack(side="left")
            tk.Label(row, text=f" {s}", font=self.f_body,
                     bg=C["symptom_bg"], fg=C["symptom_txt"], anchor="w").pack(side="left", pady=6)

        tk.Frame(body, bg=C["border"], height=1).pack(fill="x", pady=12)

        # Description
        tk.Label(body, text="ABOUT THIS DISEASE",
                 font=self.f_label, bg=C["result_pos"], fg=C["text_dim"]).pack(anchor="w")
        tk.Label(body, text=info["description"],
                 font=self.f_body, bg=C["result_pos"], fg=C["text"],
                 wraplength=680, justify="left", anchor="w").pack(fill="x", pady=(4, 12))

        # Recommendation
        rec_frame = tk.Frame(body, bg="#1A2D10",
                             highlightthickness=1, highlightbackground=C["accent_dark"])
        rec_frame.pack(fill="x")
        tk.Label(rec_frame, text="  💊 RECOMMENDATION",
                 font=self.f_label, bg="#1A2D10", fg=C["accent"]).pack(anchor="w", padx=12, pady=(10, 4))
        tk.Label(rec_frame, text=info["recommendation"],
                 font=self.f_body, bg="#1A2D10", fg=C["text"],
                 wraplength=680, justify="left", anchor="w").pack(padx=16, pady=(0, 12))
    ...
    def _fatal(self, msg):
        ...
        tk.Label(self.content,
                 ...
                 font=self.f_disease, bg=C["bg"], fg=C["danger"]).pack(pady=40)
        tk.Label(self.content, text=msg,
                 font=self.f_body, bg=C["bg"], fg=C["text"],
                 wraplength=600).pack(pady=10)
        tk.Label(self.content,
                 text=f"Ensure SWI-Prolog is installed and kb.pl is at:\n{KB_PATH}",
                 font=self.f_small, bg=C["bg"], fg=C["text_dim"]).pack(pady=10)
